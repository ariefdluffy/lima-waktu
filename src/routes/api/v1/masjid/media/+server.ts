...
export const GET: RequestHandler = async (event) => {
  ...
  const masjidId = await resolveMasjidIdForUser(user, event.url.searchParams.get("masjid_id"));
  const rows = await db
    .select()
    .from(mediaAssets)
...
export const POST: RequestHandler = async (event) => {
  ...
  if (!(file instanceof File)) {
    return json({ ok: false, message: "Field 'file' wajib diisi" }, { status: 400 });
  }

  if (!ALLOWED_MIME.includes(file.type)) {
    return json(
      { ok: false, message: "Tipe file tidak diizinkan. Gunakan JPEG, PNG, WebP, atau GIF." },
      { status: 400 }
    );
  }

  if (file.size > MAX_SIZE_BYTES) {
    return json({ ok: false, message: "Ukuran file maksimal 5MB." }, { status: 400 });
  }
