<script lang="ts">
    ...
    type BulkSchedule = {
        date: string; imsakTime: string; subuhTime: string; sunriseTime: string;
        dhuhaTime: string; dzuhurTime: string; asharTime: string; maghribTime: string; isyaTime: string;
    };
    ...
    function onKotaInput() {
        ...
        kotaResults = []; searchError = "";
        if (!kotaKeyword || kotaKeyword.length < 2) return;
        searchTimeout = setTimeout(async () => {
            searchLoading = true;
            try {
                const res = await fetch("/api/v1/prayer/search-city?q=" + encodeURIComponent(kotaKeyword));
                const json = await res.json();
                if (json.ok) kotaResults = json.data ?? [];
                else searchError = json.message ?? "Gagal mencari kota.";
            } catch { searchError = "Gagal terhubung ke server."; }
            finally { searchLoading = false; }
        }, 400);
    }

    function selectKota(kota: KotaResult) {
        selectedKota = kota; kotaKeyword = kota.lokasi; kotaResults = [];
    }
    ...
    async function fetchBulk() {
        ...
        bulkLoading = true; bulkError = ""; bulkPreview = []; bulkSuccess = ""; bulkSaveSuccess = ""; bulkSaveError = "";
        try {
            let url = "";
            if (data.prayerProviderInfo?.supportsSearch) {
                if (!selectedKota) { bulkError = "Pilih kota terlebih dahulu."; return; }
                url = "/api/v1/prayer/bulk?provider=" + provider + "&cityId=" + selectedKota.id + "&month=" + bulkMonth + "&year=" + bulkYear;
            } else {
                if (!data.masjid?.latitude || !data.masjid?.longitude) { bulkError = "Koordinat masjid belum diisi."; return; }
                url = "/api/v1/prayer/bulk?provider=" + provider + "&lat=" + data.masjid.latitude + "&lon=" + data.masjid.longitude + "&month=" + bulkMonth + "&year=" + bulkYear + "&timezone=" + (data.masjid.timezone || "Asia/Jakarta");
            }
            const res = await fetch(url);
            const json = await res.json();
            if (!json.ok) { bulkError = json.message ?? "Gagal mengambil jadwal."; return; }
            bulkPreview = json.data ?? [];
            const locationLabel = data.prayerProviderInfo?.supportsSearch ? selectedKota?.lokasi : (data.masjid?.city ?? "lokasi masjid");
            const monthLabel = MONTH_NAMES[parseInt(bulkMonth) - 1];
            bulkSuccess = bulkPreview.length + " jadwal berhasil diambil untuk " + locationLabel + " - " + monthLabel + " " + bulkYear;
        } catch { bulkError = "Gagal terhubung ke server."; }
        finally { bulkLoading = false; }
    }

    async function saveBulk() {
        if (!data.masjid || bulkPreview.length === 0) return;
        bulkSaving = true; bulkSaveSuccess = ""; bulkSaveError = "";
        try {
            const formData = new FormData();
            formData.set("masjid_id", data.masjid.id);
            formData.set("schedules", JSON.stringify(bulkPreview));
            const res = await fetch("?/bulkImportPrayerSchedule", { method: "POST", body: formData });
            const result = await res.json();
            const saved = (result as any)?.data?.saved ?? bulkPreview.length;
            bulkSaveSuccess = saved + " jadwal berhasil disimpan.";
            bulkPreview = [];
            await invalidate(() => true);
        } catch { bulkSaveError = "Gagal menyimpan jadwal."; }
        finally { bulkSaving = false; }
    }

    let scheduleDate = $state("");
    let imsakTime = $state(""); let subuhTime = $state(""); let sunriseTime = $state("");
    let dhuhaTime = $state(""); let dzuhurTime = $state(""); let asharTime = $state("");
    let maghribTime = $state(""); let isyaTime = $state("");
    ...
    async function fetchManual() {
        const provider = data.prayerProviderInfo?.providerKey ?? "myquran";
        manualFetchLoading = true; manualFetchError = ""; manualFetchSuccess = "";
        try {
            let url = "";
            if (data.prayerProviderInfo?.supportsSearch) {
                if (!selectedKota) { manualFetchError = "Pilih kota terlebih dahulu."; return; }
                url = "/api/v1/prayer/daily?provider=" + provider + "&cityId=" + selectedKota.id + "&date=" + scheduleDate;
            } else {
                if (!data.masjid?.latitude || !data.masjid?.longitude) { manualFetchError = "Koordinat masjid belum diisi."; return; }
                url = "/api/v1/prayer/daily?provider=" + provider + "&lat=" + data.masjid.latitude + "&lon=" + data.masjid.longitude + "&date=" + scheduleDate + "&timezone=" + (data.masjid.timezone || "Asia/Jakarta");
            }
            const res = await fetch(url);
            const json = await res.json();
            if (!json.ok) { manualFetchError = json.message ?? "Gagal mengambil jadwal."; return; }
            const d = json.data;
            imsakTime = d.imsakTime ?? ""; subuhTime = d.subuhTime ?? ""; sunriseTime = d.sunriseTime ?? "";
            dhuhaTime = d.dhuhaTime ?? ""; dzuhurTime = d.dzuhurTime ?? ""; asharTime = d.asharTime ?? "";
            maghribTime = d.maghribTime ?? ""; isyaTime = d.isyaTime ?? "";
            manualFetchSuccess = "Data berhasil diambil dari API.";
        } catch { manualFetchError = "Gagal terhubung ke server."; }
        finally { manualFetchLoading = false; }
    }
</script>
...
<section class="rounded-2xl bg-white p-5 shadow-sm">
    ...
    {#if scheduleActionSuccess}
        <div class="mt-3 rounded-lg bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-700">
            ✓ {scheduleActionSuccess}
        </div>
    {/if}
    {#if scheduleActionError}
        <div class="mt-3 rounded-lg bg-red-50 px-3 py-2 text-xs font-medium text-red-600">
            ✗ {scheduleActionError}
        </div>
    {/if}
...
<section class="rounded-2xl bg-white p-6 shadow-sm">
    ...
    <div class="mt-5 rounded-xl border border-slate-100 bg-slate-50 p-4">
        <p class="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-500">
            Sumber Data — {data.prayerProviderInfo?.providerName ?? "MyQuran API"}
        </p>
        {#if data.prayerProviderInfo?.supportsSearch}
            <div class="relative">
        ...
        {:else}
            <div class="rounded-lg bg-blue-50 border border-blue-200 p-3">
                <p class="text-xs text-blue-700">
                    <strong>{data.prayerProviderInfo?.providerName}</strong> menggunakan koordinat lokasi masjid.
                    {#if data.masjid?.latitude && data.masjid?.longitude}
                        <span class="block mt-1">
                            Lokasi: <strong>{data.masjid.latitude}, {data.masjid.longitude}</strong>
                            (Timezone: {data.masjid.timezone || "Asia/Jakarta"})
                        </span>
                    {:else}
                        <span class="block mt-1 text-red-600 font-semibold">
                            ⚠️ Koordinat masjid belum diisi. Silakan lengkapi di menu Profil.
                        </span>
                    {/if}
                </p>
            </div>
        ...
        <div class="mt-5 space-y-4">
            <div class="grid gap-3 sm:grid-cols-3">
                <div>
                    ...
                    <select id="bulkMonth" bind:value={bulkMonth} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100">
                ...
                <div>
                    ...
                    <select id="bulkYear" bind:value={bulkYear} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100">
                        {#each YEARS as y}
                ...
                <div class="flex items-end">
                    <button type="button" onclick={fetchBulk} disabled={bulkLoading || (data.prayerProviderInfo?.supportsSearch ? !selectedKota : !data.masjid?.latitude || !data.masjid?.longitude)} class="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50">
                        {#if bulkLoading}
            ...
            {#if bulkPreview.length > 0}
                <div class="overflow-hidden rounded-xl border border-slate-200">
                    <div class="flex items-center justify-between gap-3 border-b border-slate-200 bg-emerald-50 px-4 py-3">
                        ...
                        <button type="button" onclick={saveBulk} disabled={bulkSaving} class="flex shrink-0 items-center gap-1.5 rounded-lg bg-emerald-600 px-4 py-1.5 text-xs font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-50">
                            {#if bulkSaving}
        ...
        <div class="mt-5 space-y-4">
            <div class="flex flex-wrap items-end gap-3">
                <div class="flex-1">
                    ...
                    <input id="scheduleDateManual" type="date" bind:value={scheduleDate} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"/>
                </div>
                <button type="button" onclick={fetchManual} disabled={manualFetchLoading || !scheduleDate || (data.prayerProviderInfo?.supportsSearch ? !selectedKota : !data.masjid?.latitude || !data.masjid?.longitude)} class="flex items-center gap-2 rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-2 text-sm font-medium text-emerald-700 transition hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-50">
            ...
            <form method="POST" action="?/addPrayerSchedule" use:enhance={() => { return async ({ update }) => { await update({ reset: true }); await invalidate(() => true); }; }} class="rounded-xl border border-slate-100 bg-slate-50 p-4">
                ...
                <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <div>
                        ...
                        <input id="imsakTime" type="time" name="imsakTime" required bind:value={imsakTime} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"/>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500" for="subuhTime">Subuh</label>
                        <input id="subuhTime" type="time" name="subuhTime" required bind:value={subuhTime} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"/>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500" for="sunriseTime">Syuruq</label>
                        <input id="sunriseTime" type="time" name="sunriseTime" required bind:value={sunriseTime} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"/>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500" for="dhuhaTime">Dhuha</label>
                        <input id="dhuhaTime" type="time" name="dhuhaTime" required bind:value={dhuhaTime} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"/>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500" for="dzuhurTime">Dzuhur</label>
                        <input id="dzuhurTime" type="time" name="dzuhurTime" required bind:value={dzuhurTime} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"/>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500" for="asharTime">Ashar</label>
                        <input id="asharTime" type="time" name="asharTime" required bind:value={asharTime} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"/>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500" for="maghribTime">Maghrib</label>
                        <input id="maghribTime" type="time" name="maghribTime" required bind:value={maghribTime} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"/>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500" for="isyaTime">Isya</label>
                        <input id="isyaTime" type="time" name="isyaTime" required bind:value={isyaTime} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"/>
                    </div>
                </div>
                <div class="mt-4 flex items-center justify-between">
                    <p class="text-xs text-slate-400">Jadwal akan disimpan untuk tanggal yang dipilih di atas</p>
                    <button type="submit" class="flex items-center gap-2 rounded-xl bg-emerald-600 px-5 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700">
