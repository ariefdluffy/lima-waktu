...
<article class="rounded-2xl bg-white p-5 shadow-sm">
    ...
    <div class="mt-4 space-y-3">
            ...
            <div class="rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3">
                <details class="group">
                    <summary class="flex cursor-pointer items-center justify-between gap-2">
                        <p class="flex-1 truncate text-sm text-slate-700">{item.content}</p>
                    ...
                    <div class="mt-3 space-y-2 border-t border-emerald-100 pt-3">
                        <form method="POST" action="?/editRunningText" use:enhance={refresh} class="space-y-2">
                            ...
                            <div class="flex items-center gap-2">
                                <label for="speed_{item.id}" class="text-xs text-slate-500">Speed (detik):</label>
                                <input
                                    id="speed_{item.id}"
                                    type="number"
                                    name="speed"
        ...
        {#if data.runningTexts.length === 0}
            <p class="text-xs text-slate-400">Belum ada running text.</p>
        {/if}
    </div>
    <Pagination
