...
export function renderResetPasswordHtml(props: ResetPasswordTemplateProps): string {
  return `<!DOCTYPE html>
<html lang="id">
    ...
    <tr><td align="center" style="padding:40px 16px">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.06)">
        ...
        <tr><td style="padding:24px 40px;text-align:center">
          <a href="${props.resetUrl}" style="display:inline-block;padding:12px 32px;background:#0A3D2E;color:#fff;text-decoration:none;font-size:15px;font-weight:600;border-radius:6px">Reset Password</a>
        </td></tr>
        <tr><td style="padding:0 40px 32px;text-align:center">
          <p style="margin:0;font-size:13px;color:#888;line-height:1.5">Atau buka link berikut:<br><a href="${props.resetUrl}" style="color:#C8A84B;font-size:13px">${props.resetUrl}</a></p>
          <p style="margin:8px 0 0;font-size:12px;color:#aaa">Link ini berlaku selama ${props.expiresInHours} jam.</p>
        </td></tr>
        <tr><td style="padding:0 40px 24px;text-align:center">
          <p style="margin:0;font-size:12px;color:#bbb;line-height:1.4">Jika Anda tidak meminta reset password, abaikan email ini.</p>
        </td></tr>
